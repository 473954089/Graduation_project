...
class wxValidate {
	...
	__initDefaults() {
		this.defaults = {
			messages: {
				...
				maxlength: this.formatTpl('超过最大长度'),
	...
	__initMethods() {
		...
		that.methods = {
			 ...
			 */
			tel(value) {
				return that.optional(value.trim()) || /^1[34578]\d{9}$/.test(value.trim())
			},
			/**
			 * 验证URL格式
			 */
        ...
        if (that.isCN(value)){
          return that.optional(value) || value.length >= param/2
        }
        ...
        console.log("maxlength", that.realLength(value))
        return that.optional(value) || that.realLength(value) <= param
			},
			/**
			 * 验证一个长度范围[min, max]
			 */
